...
> 你不做评审、不做门禁裁定——评审由外部模型（deepseek/kimi）做，质检编排由 Claude Code 做。
...
6. **写三件套**（格式见下）。
7. **等待门禁**：写完后停止，等 Claude Code 侧跑 L0/L2/L3 门禁。
...
- 写完后即视为「提交草稿」，等 Claude Code 生成 `projects/02_modeling/qa/M1.json`。
- 若收到 `M1.json` 中 `rework_suggestions` 的打回建议，**直接修订三件套**（不新建平行版本），在报告里说明变更，再重新提交重检。
- 门禁 `status` 为 `PASS` 才进入编码阶段。
